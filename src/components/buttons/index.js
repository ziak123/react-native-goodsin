...
export const ButtonBorderd = ({ text = '', onPress, bgColor, background, icon, textColor, style, isLoading, textStyle }) => {
    return (
        <TouchableOpacity activeOpacity={.8}
            disabled={isLoading}
            style={[styles.buttonBorder, { backgroundColor: bgColor ?? colors.appBgColor1, borderColor: background ?? colors.appBorderColor1 }, style]}
            onPress={onPress}>
...
export const ButtonColored = ({ text = '', onPress, disabled, background, textColor, animation, duration, withIcon, style, isLoading, textStyle }) => {
    return (
        <Wrapper animation={animation} duration={duration} >
            <TouchableOpacity activeOpacity={.8}
                disabled={disabled}
                style={[styles.buttonColored, { backgroundColor: background ? background : disabled ? colors.appBgColor4 : colors.appBgColor4, }, style]}
                onPress={onPress}>
                {withIcon ? <RowWrapperBasic >
                    ...
                    :
                    isLoading ? <ActivityIndicator color={colors.appTextColor3} />
                        : <Text style={[styles.coloredButtonText, { color: textColor ?? colors.appTextColor2 }, textStyle]}>{text}</Text>}
            </TouchableOpacity>
        </Wrapper>
    )
}
